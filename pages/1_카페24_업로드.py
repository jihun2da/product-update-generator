...
st.success(
    "✅ 실제 카페24 계정으로 '로그인 → 업로드 화면 이동 → 파일 업로드 → 성공/실패 판별'까지 "
    "전체 흐름 테스트를 완료했고, 정상 동작하는 것을 확인했습니다.\n\n"
    "- 실패한 계정은 재시도하지 않고 끝까지 진행한 뒤, 실패한 계정 이름을 모아서 알려드립니다.\n"
    "- 계정별 스크린샷은 '④ 업로드 결과'에서 계속 확인할 수 있으니, 새로운 계정이나 파일을 "
    "추가하실 때는 처음 한 번씩 결과를 확인해보시는 걸 권장드립니다.\n"
