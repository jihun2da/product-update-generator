...
MODULE_VERSION = "2026-08-27-6 (정식 버전: 코드/문서 정리, 중복 코드 제거)"
...
# "엑셀 업로드" 버튼을 클릭하면 이 URL 조각을 포함한 요청(POST
# .../exec/admin/product/ProductExcelSet)이 발생합니다. 이 요청의 응답이 실제로
# 올 때까지 직접 기다리기 위한 기준값으로 사용합니다(아래 submit_wait_sec 참고).
...
_DISMISS_POPUP_SELECTORS = [
    ...
    # 실제 화면에서 확인한 경로이며, 다른 선택자보다 먼저 시도합니다.
...
def _close_popups(context, page, dialog_messages=None, popup_page_log=None, click_state=None):
    ...
    직전에도 호출합니다 — "전용 엑셀양식 다운로드" 같은 안내 팝업이 업로드 화면에서
    새로 뜨면서 업로드 버튼을 가려 클릭이 안 먹히는 경우가 있기 때문입니다. 클릭
    자체가 끝까지 실패하면 성공으로 처리하지 않고 오류로 보고합니다(아래
    upload_one_account 참고).

    카페24의 업로드 확인창("해당 파일을 업로드 하시겠습니까?")이 네이티브 confirm()
    대화상자가 아니라 window.open()으로 열리는 별도의 팝업 페이지에 HTML로 그려지는
    경우가 있어(네이티브 dialog는 window.confirm 오버라이드 + context.on("dialog")로
    별도 처리), 다른 페이지를 닫기 전에 반드시 먼저 그 안에서 "확인"류 버튼을 찾아
    클릭을 시도하고, 성공하면 그 페이지를 곧바로 닫지 않습니다(확인으로 촉발된 실제
    업로드 요청이 그 페이지에서 진행 중일 수 있으므로, 강제로 끊지 않기 위함). "확인"
    버튼을 끝내 찾지 못한 경우에만(=업로드 확인창이 아니라 진짜 광고/안내성 팝업으로
    판단) 기존처럼 닫기 버튼을 시도한 뒤 정리합니다.
    """
    time.sleep(1)
...
def upload_one_account(playwright, name, cafe24_id, cafe24_pw, file_bytes, file_name,
    ...
    network_log = []
    console_log = []
    # try 블록 진입 직후 예외가 나도(예: 브라우저 실행 자체 실패) AccountResult에
    # 빈 로그를 안전하게 담을 수 있도록 여기서 미리 초기화해둡니다.
    popup_page_log = []
    temp_path = None
    try:
        ...
        #
        # 이를 막기 위해 window.confirm()이 "네이티브 확인창을 띄우지 않고" 항상
        # 즉시 확인(true)을 반환하도록, 페이지의 스크립트가 실행되기도 전에 미리
        # 덮어씁니다(context.add_init_script). 이 방식은 Playwright의 다이얼로그
        # 이벤트 타이밍과 무관하고(다이얼로그 자체가 아예 생기지 않음),
        # headless/headed 어느 쪽이든 메인 페이지든 이후 생기는 팝업 페이지든 항상
        # 동일하게 동작합니다. 혹시 모를 다른 종류의 다이얼로그(예: 이 오버라이드가
        # 안 걸리는 특수한 경우)에 대비해서, 기존 context.on("dialog") 핸들러도
        # 이중 안전장치로 남겨뒀습니다.
        dialog_messages = []
        ...
        # 위 window.confirm 오버라이드는 "네이티브 자바스크립트 대화상자"에만
        # 적용됩니다. 카페24의 업로드 확인창이 네이티브 dialog가 아니라
        # window.open()으로 열리는 별도의 팝업 페이지에 HTML로 그려지는 경우도 있어,
        # 새로운 페이지가 생성되는 즉시(page 이벤트) 그 안에서 "확인"류 버튼을 찾아
        # 자동으로 클릭을 시도하는 안전장치를 추가합니다.
        ...
        # "업로드 버튼을 눌렀을 때 실제로 무슨 일이 일어나는지"를 추측이 아니라
        ...
        #
        # 위 다이얼로그 핸들러와 같은 이유로, 이 계측도 page 하나가 아니라 context
        # 레벨에 등록합니다 — 실제 업로드 요청이 팝업 페이지 쪽에서 발생하는 경우까지
        # 포함해, 메인 페이지든 이후 생기는 어떤 팝업이든 상관없이 콘솔/요청/응답을
        # 전부 놓치지 않고 기록하기 위함입니다.
        ...
        # 중요: 첨부한 임시 파일(temp_path)은 여기서 지우지 않습니다. set_input_files()는
        # 파일을 그 자리에서 읽어 메모리에 담아두는 게 아니라 "이 경로의 파일을 나중에
        # 제출 시점에 읽어라"는 참조만 저장해두는 방식이라서, 여기서 바로 삭제하면
        # 몇 초 뒤 실제로 업로드 버튼을 눌러 폼이 제출될 때 파일이 이미 없어
        # net::ERR_FILE_NOT_FOUND로 요청이 실패합니다. 이 계정의 업로드 시도가 완전히
        ...
        # 증상으로 나타납니다. 버튼을 누르기 전에 먼저 (1) 파일 검증 관련 네트워크
        # 활동이 잠잠해질 때까지 짧게 기다리고 (2) 화면이 완전히 전환될 여유를 위해
        # 추가로 잠깐 더 기다립니다(둘 다 실패해도 예외를 삼키고 그냥 진행 — 무한 대기
        # 방지).
        try:
        ...
        if not post_click_requests:
            ...
            # 응답이 안 온 게 "서버가 느린 것"인지 "요청 자체가 중간에
